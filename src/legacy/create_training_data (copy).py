...
if options.include_lgg == True:
    pat_range = range(0, train_split_hgg + train_split_lgg)
else:
    pat_range = range(0, train_split_hgg)
...
if options.shuffle_data == True:
    ...
    import random
    random.seed(3)
    random.shuffle(pat_range)

# populating x_train with HGG data first
run_count = 0
logger.info('Running on HGG Data.')
for pat in pat_range: # 210 patients in HGG

    if pat >= train_split_hgg: # this is an LGG index, so load data from LGG
        loadFromData = 'training_data_lgg'
        loadFromPatName = 'training_data_lgg_pat_name'
        loadFromytrain = 'training_data_segmasks_lgg'
    else:
        loadFromData = 'training_data_hgg'
        loadFromPatName = 'training_data_hgg_pat_name'
        loadFromytrain = 'training_data_segmasks_hgg'

    hdf5_file_out['x_train'][run_count] = hdf5_file[loadFromData][pat]
    hdf5_file_out['training_data_pat_name'][run_count] = hdf5_file[loadFromPatName][pat]

    if options.ground_truth == 'seg':
        hdf5_file_out['y_train'][run_count] = hdf5_file[loadFromytrain][pat]
    else:
        rect_obj = bbox_3D(hdf5_file[loadFromytrain][pat, ...], tol=0.1)
