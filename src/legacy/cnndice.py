...
def bbox_overlap_iou(y_true, y_pred):
    ...
    # current run is negative. So flip the sign on the console display
    return -iou
...
def dice_coefficient(y_true, y_pred):
    ...
    dice = 2 * intersection / (vol_true + vol_pred)
    return dice
...
loss_function_dict = {iou_with_mse.__name__: iou_with_mse,
                      mean_absolute_error.__name__: mean_absolute_error,
                      bbox_overlap_iou.__name__: bbox_overlap_iou}
hyparams['loss_function'] = iou_with_mse.__name__
hyparams['optimizer'] = 'adam'
...
hyparams[bname]['out_activation'] = 'relu'
...
def compile_model(model):
    model.compile(optimizer=hyparams['optimizer'], loss=loss_function_dict[hyparams['loss_function']],
                  metrics=[bbox_overlap_iou, mean_absolute_error])
    return model
