...
config['hdf5_filepath_cropped'] = os.path.join(mount_path_prefix, 'scratch/asa224/Datasets/BRATS2017/HDF5_Datasets/BRATS2017_Cropped_Normalized.h5') # top level path
config['saveMeanVarFilepathHGG'] = os.path.join(os.sep.join(config['hdf5_filepath_prefix'].split(os.sep)[0:-1]), 'BRATS2017_HDF5_Datasetstraining_data_hgg_mean_var.p')
config['saveMeanVarFilepathLGG'] = os.path.join(os.sep.join(config['hdf5_filepath_prefix'].split(os.sep)[0:-1]), 'BRATS2017_HDF5_Datasetstraining_data_lgg_mean_var.p')
